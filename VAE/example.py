...
from tools import load_data, compute_loss, test, vis_img, vis_lat, add_noise
...
def config_parser():
    ...
    parser.add_argument("--hid_dim", type=int, default=2000, help="latent for hidden size")
    parser.add_argument("--lat_dim", type=int, default=100, help="latent dimension")
...
for i in tqdm(range(args.epoch)):
    ...
    if (i+1) % i_interval == 0:
        vis_lat(i+1, model, args, writer)
        test(i+1, test_loader, model, args, writer)
        vis_img(i+1, vis_data.to(args.device), model, writer)
