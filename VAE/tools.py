...
def vis_lat(i, model, args, writer):
    if args.lat_dim > 2:
        # only 1 or 2 lat_dim can be visualized
        return

    # sample z from [gen_s, gen_e] for each dimension
    if args.lat_dim == 1:
        ...
        y = model.generate(torch.cat([x.unsqueeze(-1), x.unsqueeze(-1)], dim=-1).reshape(-1, 2))
        y = y.view(20, 20, 28, 28).permute(0, 2, 1, 3).reshape(20 * 28, 20 * 28).unsqueeze(0)
        writer.add_image("generate_lat", y, i)
