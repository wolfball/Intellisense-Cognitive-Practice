...
class Encoder(nn.Module):
    ...
    def forward(self, images):
        ...
        # images = [a for a in images]
        out = self.resnet(images)
        # out = self.feature_extractor(images=images, return_tensors='pt').pixel_values[0]
        # print("after resnet:", out.shape)
        # sssssss
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths,
        ...
        # prev_timestamp_words = torch.LongTensor([[pad_value]] * batch_size * encoded_captions.shape[-1]).view(
        #     encoded_captions.shape).to(encoder_out.device)
    ...
    def sample(self, encoder_out, startseq_idx, endseq_idx=-1, max_len=40,
        ...
        method = sample_kwargs.get("method", "greed")
        beam_size = sample_kwargs.get("beam_size", 5)
...
class Captioner(nn.Module):
    def __init__(self, encoded_image_size, attention_dim, embed_dim,
        ...
        super().__init__()
        # self.net = VisionEncoderDecoderModel.from_pretrained("nlpconnect/vit-gpt2-image-captioning")
        self.encoder = Encoder(encoded_image_size=encoded_image_size)
        # self.net = VisionEncoderDecoderModel.from_pretrained("nlpconnect/vit-gpt2-image-captioning")
        # self.encoder = self.net.get_encoder()
    ...
    def sample(self, images, startseq_idx, endseq_idx=-1, max_len=40,
        ...
        return self.decoder.sample(encoder_out=encoder_out,
                                   ...
                                   return_alpha=return_alpha, **sample_kwargs)